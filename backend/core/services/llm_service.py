...
class LLMService:
    ...
    def __init__(self):
        ...
        with get_tracer(__name__).start_as_current_span(
            ...
            try:
                ...
                logger.warning(
                    f"Encoding cl100k_base não encontrado. Usando split() como fallback. Erro: {e}"
                )
    ...
    def _initialize_client(self):
        ...
        with self.tracer.start_as_current_span(
            "llm_service.initialize_client", kind=SpanKind.INTERNAL
        ) as span:
            try:
    ...
    def _count_tokens(self, text: str) -> int:
        ...
        if self.tokenizer:
            try:
                ...
                logger.warning(
                    f"Erro ao usar tiktoken para contar tokens: {e}. Usando split()."
                )
    ...
    async def generate_text(
        ...
        """
        with self.tracer.start_as_current_span(
            "llm_service.generate_text", kind=SpanKind.CLIENT
        ) as span:
            start_time = time.time()

            effective_model = (
                model or self.settings.LLM_MODEL or "meta/llama3-70b-instruct"
            )
            ...
            try:
                ...
                logger.error(
                    f"Erro na chamada LLM ({effective_model}): {e}", exc_info=True
                )
                raise LLMServiceError(
                    f"Erro ao gerar texto com modelo {effective_model}: {e}"
                ) from e

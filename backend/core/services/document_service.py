...
from utils.metrics_prometheus import record_document_processing,record_chunk_size, record_extraction_time, record_chunking_quality
...
class DocumentService:
    ...
    async def process_document(self, file_name: str, file_content: bytes, 
        ...
        with self.tracer.start_as_current_span(
            ...
            span.set_attribute("document.initial_metadata_keys", str(list(metadata.keys()) if metadata else []))
            ...
            try:
                ...
                with self.tracer.start_as_current_span("db.save_initial_document", kind=SpanKind.CLIENT) as db_save_span:
                    ...
                    db_save_span.set_attribute(SpanAttributes.DB_SQL_TABLE, "documentos_originais")
                    db_save_span.set_attribute("document.name", document.name) # Repetir aqui para contexto do DB
                    try:
                        ...
                        if db_document_id:
                             ...
                             db_save_span.set_status(Status(StatusCode.ERROR, "Falha ao obter ID do documento após salvar."))
                             # Métrica Prometheus (manter)
                             record_document_processing('error_db_save', file_type)
                             raise DocumentProcessingError(f"Erro ao salvar documento {file_name} no DB (sem ID retornado)")
                    except Exception as e:
                        db_save_span.record_exception(e)
                        db_save_span.set_status(Status(StatusCode.ERROR, str(e)))
                        record_document_processing('error_db_save', file_type) # Métrica
                        raise # Relançar para ser pego pelo try/except principal

                document.id = db_document_id
                span.set_attribute("db.document_id", db_document_id) # Adicionar ID ao span principal
                ...
                if chunk_strategy == "auto":
                    ...
                    span.set_attribute("document.chunk_strategy_effective", suggested_strategy)
                else:
                    span.set_attribute("document.chunk_strategy_effective", chunk_strategy)
                ...
                if not chunks:
                    ...
                    span.set_status(Status(StatusCode.OK, "Processamento concluído, mas sem chunks gerados."))
                    document.processed = False # Marcar como não processado (sem chunks)
                    return document
                ...
                record_chunking_quality(
                    quality_metrics["avg_coherence"], # Assumindo que avg_coherence é o score principal
                    suggested_strategy,
                    file_type
                )
                ...
                for i, (chunk_text, embedding) in enumerate(zip(chunks, embeddings)):
                    ...
                    chunk_metadata = {
                        ...
                        "chunk_quality_coherence": quality_metrics.get('avg_coherence') # Salvar score específico
                ...
                record_document_processing('success_complete', file_type) # Usar função auxiliar
                ...
                span.set_attribute("document.avg_chunk_length", sum(len(ct) for ct in chunks) / max(1, len(chunks)))
                ...
                span.set_status(Status(StatusCode.OK)) # Marcar OK no span principal se chegou até aqui

                return document

            except Exception as e:
                processing_time = time.time() - processing_start # Calcular tempo mesmo em erro
                logger.error(f"Erro ao processar documento {file_name} (ID: {db_document_id}): {e}", exc_info=True)
                ...
                raise DocumentProcessingError(f"Erro ao processar documento {file_name}: {str(e)}") from e
    ...
    async def get_document(self, document_id: int) -> Optional[Document]:
        ...
        """
        with self.tracer.start_as_current_span("document_service.get_document", kind=SpanKind.SERVER) as span:
    ...
    async def list_documents(self, include_content: bool = False) -> List[Document]:
        ...
        """
        with self.tracer.start_as_current_span("document_service.list_documents", kind=SpanKind.SERVER) as span:
    ...
    async def delete_document(self, document_id: int) -> bool:
        ...
        """
        with self.tracer.start_as_current_span("document_service.delete_document", kind=SpanKind.SERVER) as span:
             span.set_attribute("db.document_id", document_id)
             try:
    ...
    async def search_documents(self, query: str) -> List[Document]:
        ...
        """
        with self.tracer.start_as_current_span("document_service.search_documents", kind=SpanKind.SERVER) as span:

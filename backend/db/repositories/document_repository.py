...
class DocumentoRepository:
    ...
    @staticmethod
    def criar_documento(nome_arquivo: str, tipo_arquivo: str, conteudo_binario: bytes, 
        ...
        try:
            result = execute_query_single_result(
                ...
                RETURNING id
                """,
                (nome_arquivo, tipo_arquivo, Binary(conteudo_binario), Json(metadados or {}))
            )
    ...
    def atualizar_metadados(documento_id: int, metadados: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(f"Metadados do documento {documento_id} atualizados com sucesso")
            return True
            
        except Exception as e:
            logger.error(f"Erro ao atualizar metadados do documento {documento_id}: {e}")
            return False

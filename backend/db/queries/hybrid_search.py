...
def realizar_busca_hibrida(
    ...
    with tracer.start_as_current_span(
        ...
        span.set_attribute(
            "app.search.query_text", query_text
        )  # Usar prefixo 'app' para dados da aplicação
        ...
        span.set_attribute(
            "app.search.threshold", threshold or get_settings().SEARCH_THRESHOLD
        )
        ...
        effective_threshold = (
            threshold if threshold is not None else settings.SEARCH_THRESHOLD
        )
        span.set_attribute(
            "app.search.effective_threshold", effective_threshold
        )  # Registrar threshold efetivo
        ...
        try:
            ...
            span.set_attribute(
                "db.sql.filter_clause", filter_clause_sql
            )  # Adicionar cláusula ao span
            ...
            vector_params = (
                [query_embedding]
                + filter_params_vector
                + [query_embedding, initial_limit]
            )
            text_params = (
                [query_text, query_text] + filter_params_text + [initial_limit]
            )
            ...
            with tracer.start_as_current_span(
                ...
                vector_query_formatted = VECTOR_SEARCH_QUERY.format(
                    filter_clause=filter_clause_sql
                )
                # Adicionar atributos semânticos do DB
                vector_span.set_attribute(
                    SemanticSpanAttributes.DB_SYSTEM, "postgresql"
                )
                vector_span.set_attribute(
                    SemanticSpanAttributes.DB_OPERATION, "SELECT vector_similarity"
                )
                vector_span.set_attribute(
                    SemanticSpanAttributes.DB_STATEMENT, VECTOR_SEARCH_QUERY
                )  # Query template
                vector_span.set_attribute(
                    SemanticSpanAttributes.DB_SQL_TABLE, "chunks_vetorizados"
                )
                ...
                try:
                    ...
                    vector_span.set_status(
                        Status(StatusCode.ERROR, description=str(db_exc))
                    )
            ...
            text_rows = []
            with tracer.start_as_current_span(
                "db.text_search", kind=SpanKind.CLIENT
            ) as text_span:
                text_filter_clause_sql = (
                    f"AND {filter_clause_sql}" if filter_clause_sql else ""
                )  # Ajuste da lógica original
                text_query_formatted = TEXT_SEARCH_QUERY.format(
                    filter_clause=text_filter_clause_sql
                )  # Usar versão formatada da cláusula
                # Adicionar atributos semânticos do DB
                text_span.set_attribute(SemanticSpanAttributes.DB_SYSTEM, "postgresql")
                text_span.set_attribute(
                    SemanticSpanAttributes.DB_OPERATION, "SELECT text_rank"
                )
                text_span.set_attribute(
                    SemanticSpanAttributes.DB_STATEMENT, TEXT_SEARCH_QUERY
                )  # Query template
                text_span.set_attribute(
                    SemanticSpanAttributes.DB_SQL_TABLE, "chunks_vetorizados"
                )
            ...
            with tracer.start_as_current_span(
                "db.process_hybrid_results", kind=SpanKind.INTERNAL
            ) as process_span:
                ...
                for row in text_rows:
                    ...
                    if chunk_id in combined_results:
                        ...
                        with tracer.start_as_current_span(
                            "db.get_similarity_for_text_chunk", kind=SpanKind.CLIENT
                        ) as sim_span:
                            sim_span.set_attribute(
                                "app.chunk_id", chunk_id
                            )  # ID do chunk buscado
                            sim_span.set_attribute(
                                SemanticSpanAttributes.DB_SYSTEM, "postgresql"
                            )
                            sim_span.set_attribute(
                                SemanticSpanAttributes.DB_OPERATION,
                                "SELECT vector_similarity",
                            )
                            sim_span.set_attribute(
                                SemanticSpanAttributes.DB_STATEMENT,
                                GET_SIMILARITY_SCORE_QUERY_TEMPLATE,
                            )  # Usar template
                            sim_span.set_attribute(
                                SemanticSpanAttributes.DB_SQL_TABLE,
                                "chunks_vetorizados",
                            )
                            ...
                            try:
                                ...
                                if (
                                    similarity_row
                                    and "similarity_score" in similarity_row
                                ):
                                    similarity_score = float(
                                        similarity_row["similarity_score"]
                                    )
                                sim_span.set_attribute(
                                    "db.result_score", similarity_score
                                )
                                ...
                                sim_span.set_attribute(
                                    "error.type", type(db_exc).__name__
                                )
                ...
                process_span.set_attribute(
                    "results.new_chunks_from_text", new_chunks_from_text
                )
                process_span.set_attribute(
                    "results.similarity_lookups", similarity_lookup_count
                )
                process_span.set_attribute(
                    "results.combined_initial_count", len(combined_results)
                )
                ...
                for chunk in combined_results.values():
                    ...
                    norm_text_score = min(
                        getattr(chunk, "text_score", 0.0), 1.0
                    )  # Usar getattr com default

                    # Combinar scores
                    chunk.combined_score = (
                        alpha * chunk.similarity_score + (1 - alpha) * norm_text_score
                    )
                ...
                filtered_chunks = [
                    chunk
                    for chunk in combined_results.values()
                    if chunk.combined_score >= effective_threshold
                ]
                retained_count = len(filtered_chunks)
                filtered_count = total_resultados_before_threshold - retained_count
                process_span.set_attribute(
                    "results.before_threshold_count", total_resultados_before_threshold
                )
                process_span.set_attribute(
                    "results.after_threshold_count", retained_count
                )
                process_span.set_attribute(
                    "results.filtered_by_threshold_count", filtered_count
                )
                ...
                sorted_chunks = sorted(
                    filtered_chunks, key=lambda x: x.combined_score, reverse=True
                )[:limite]
                process_span.set_attribute("results.final_count", len(sorted_chunks))
                if sorted_chunks:
                    process_span.set_attribute(
                        "results.top_score", sorted_chunks[0].combined_score
                    )
                    process_span.set_attribute(
                        "results.lowest_score", sorted_chunks[-1].combined_score
                    )
...
def rerank_results(chunks: List[Chunk], query_text: str) -> List[Chunk]:
    ...
    with tracer.start_as_current_span(
        ...
        for chunk in chunks:
            with tracer.start_as_current_span(
                "db.rerank_apply_heuristics"
            ) as heuristic_span:
                heuristic_span.set_attribute("app.chunk_id", chunk.id)
                original_score = chunk.combined_score
                heuristic_span.set_attribute(
                    "app.rerank.original_score", original_score
                )
                ...
                if query_text.lower() in chunk.texto.lower():
                    ...
                    heuristic_span.set_attribute(
                        "app.rerank.heuristic.position_boost", position_boost
                    )
                ...
                if actual_length > ideal_length * 3:
                    ...
                    length_penalty = min(
                        0.03, (actual_length - ideal_length * 3) / 10000
                    )
                    heuristic_span.set_attribute(
                        "app.rerank.heuristic.length_penalty", length_penalty
                    )
                ...
                if chunk.metadados and "chunking_strategy" in chunk.metadados:
                    ...
                    heuristic_span.set_attribute(
                        "app.rerank.heuristic.chunking_strategy", strategy
                    )
                    ...
                    heuristic_span.set_attribute(
                        "app.rerank.heuristic.is_specific_query", is_specific_query
                    )
                    ...
                    heuristic_span.set_attribute(
                        "app.rerank.heuristic.strategy_boost", strategy_boost
                    )
                ...
                if chunk.metadados and "chunk_quality" in chunk.metadados:
                    ...
                    heuristic_span.set_attribute(
                        "app.rerank.heuristic.quality_boost", quality_boost
                    )

                # Aplicar todos os ajustes
                total_adjustment = (
                    position_boost + strategy_boost + quality_boost - length_penalty
                )
                chunk.combined_score = original_score * (1 + total_adjustment)

                heuristic_span.set_attribute(
                    "app.rerank.total_adjustment", total_adjustment
                )
                heuristic_span.set_attribute(
                    "app.rerank.adjusted_score", chunk.combined_score
                )

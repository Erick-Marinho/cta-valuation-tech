...
def testar_estrategias_chunking(
    arquivo_path: str, tamanho_chunk: int = 800, sobreposicao: int = 100
):
    """
    ...
    try:
        ...
        for estrategia in estrategias:
            ...
            quartil_1 = (
                tamanhos[len(tamanhos) // 4] if len(chunks) >= 4 else min_tamanho
            )
            mediana = tamanhos[len(tamanhos) // 2] if chunks else 0
            quartil_3 = (
                tamanhos[3 * len(tamanhos) // 4] if len(chunks) >= 4 else max_tamanho
            )
        ...
        print(
            f"{'Estratégia':<15} {'Chunks':<8} {'Tempo (s)':<10} {'Coerência':<10} {'Tamanho Médio':<15} {'Variação':<10}"
        )
        print("-" * 80)
        ...
        print("-" * 80)
        print(
            f"{'Estratégia':<15} {'Coerência':<10} {'Cobertura':<10} {'Consistência':<12}"
        )
        print("-" * 80)
        ...
        print("-" * 80)
        print(
            f"{'Estratégia':<15} {'Mínimo':<8} {'Q1':<8} {'Mediana':<8} {'Q3':<8} {'Máximo':<8}"
        )
        print("-" * 80)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tamanho", type=int, default=800, help="Tamanho máximo do chunk"
    )
    parser.add_argument(
        "--sobreposicao", type=int, default=100, help="Sobreposição entre chunks"
    )
    parser.add_argument(
        "--detalhe", action="store_true", help="Mostrar análise detalhada"
    )

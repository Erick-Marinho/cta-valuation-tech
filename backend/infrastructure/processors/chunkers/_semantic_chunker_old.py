...
def hybrid_chunking(text: str, chunk_size: int, chunk_overlap: int) -> List[str]:
    ...
    for section in sections:
        ...
        if len(section) <= chunk_size:
            ...
            section_chunks = paragraph_based_chunking(
...
def evaluate_chunk_quality(chunks: List[str], original_text: str) -> Dict[str, float]:
    ...
    for chunk in chunks:
        ...
        starts_with_conjunction = bool(
            re.match(r"\b(e|mas|porém|contudo|entretanto)\b", chunk.lower())
        )
...
def create_semantic_chunks(
    ...
    logger.info(
        f"Iniciando chunking semântico. Tamanho do texto: {len(text)} caracteres. Estratégia: {strategy}"
    )
    ...
    if strategy == "header_based":
        ...
        logger.warning(
            f"Estratégia desconhecida: {strategy}. Usando híbrida por padrão."
        )
        chunks = hybrid_chunking(text, chunk_size, chunk_overlap)
    ...
    logger.info(
        f"Chunking concluído. Gerados {len(final_chunks)} chunks. Qualidade: {quality_metrics}"
    )

...
def extract_keywords(text: str, max_keywords: int = 10) -> List[str]:
    ...
    stopwords = {
        ...
        'lhes', 'mais', 'mas', 'me', 'mesmo', 'meu', 'meus', 'minha', 'minhas', 'muito', 'na',
        'não', 'nas', 'nem', 'no', 'nos', 'nós', 'nossa', 'nossas', 'nosso', 'nossos', 'num',
        ...
        'tiveram', 'tivéramos', 'tiverem', 'tivermos', 'tu', 'tua', 'tuas', 'um', 'uma',
        'você', 'vocês', 'vos'
    }
    
    # Dividir em palavras e filtrar stopwords
    words = [w for w in re.findall(r'\b\w+\b', cleaned_text) if w not in stopwords and len(w) > 2]
...
def analyze_text(text: str) -> Dict[str, Any]:
    ...
    sentence_count = len(re.findall(r'[.!?]+', text)) + 1  # +1 para incluir a última sentença
    ...
    pt_indicators = ['de', 'que', 'para', 'com', 'não', 'uma', 'os', 'no', 'se', 'na', 'por', 'mais']
    en_indicators = ['the', 'and', 'to', 'of', 'in', 'is', 'that', 'for', 'you', 'with', 'on', 'by']
